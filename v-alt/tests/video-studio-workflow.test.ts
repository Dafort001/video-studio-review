...
test("supported safe source motion changes renderer frames and unsupported safe motion is explicit", () => {
  ...
  assert.match(guidedSource, /Renderer-Unterstützung ausstehend/);
...
test("AI draft persists selected status through Shared while provider generation remains disabled", () => {
  ...
  assert.match(guidedSource, /Auftragsfotos werden nicht als KI-Ergebnisse ausgegeben/);
  ...
  assert.match(guidedSource, /Generierung starten · noch gesperrt/);
...
test("normal preview remains available next to optional AI Studio", () => {
  assert.match(workbenchSource, /Normale Werkstatt-Vorschau/);
