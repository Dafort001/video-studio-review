...
import { Check, GripVertical, Redo2, Trash2, Undo2 } from "lucide-react";
...
export function MotionCatalogPanel({
  ...
  const motions = VIDEO_STUDIO_MOTION_CATALOG.filter(
    (motion) => motion.sourceGroup === group && `${motion.label} ${motion.id}`.toLowerCase().includes(query.toLowerCase()),
  );
  return (
    <section data-scene-tool="motion" className="space-y-4 border border-border/70 bg-card p-5">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Bewegung</p>
        <h3 className="mt-1 text-lg font-semibold">Quellenbasiert oder generativ</h3>
        <p className="mt-1 text-xs text-muted-foreground">91 erweiterbare Bewegungen aus dem verbindlichen Katalog; generative Varianten werden erst nach einem Storyboard und bewusster Freigabe erzeugt.</p>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <Button type="button" variant={group === "source_based" ? "default" : "outline"} onClick={() => setGroup("source_based")}>Aus dem Original</Button>
        <Button type="button" variant={group === "generative_ai" ? "default" : "outline"} onClick={() => setGroup("generative_ai")}>Generativ / KI</Button>
      </div>
      <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Bewegung suchen" className="w-full border border-border bg-background px-3 py-2 text-sm" />
      <div className="max-h-[520px] space-y-2 overflow-y-auto pr-1">
        {(["classic", "depth", "perspective", "outpainting", "focus"] as const).map((family) => {
          const familyMotions = motions.filter((motion) => motion.family === family);
          if (!familyMotions.length) return null;
          const label = { classic: "Klassische 2D-Bewegungen", depth: "Tiefe & Parallaxe", perspective: "Neue Perspektiven", outpainting: "Bildraum erweitern", focus: "Fokus & Objekt" }[family];
          return (
            <details key={family} open={family === "classic" || family === "perspective"} className="border border-border p-2">
              <summary className="cursor-pointer px-1 py-2 text-xs font-bold uppercase tracking-wide">{label} · {familyMotions.length}</summary>
              <div className="space-y-2 pt-2">
                {familyMotions.map((motion) => (
                  <button key={motion.id} type="button" disabled={isSelectable ? !isSelectable(motion) : false} onClick={() => onSelect(motion)} className={`w-full border p-3 text-left disabled:cursor-not-allowed disabled:opacity-45 ${selectedMotionId === motion.id ? "border-primary bg-primary/5" : "border-border"}`}>
                    <span className="text-sm font-semibold">{motion.label}</span>
                    <span className="mt-1 block text-[11px] uppercase tracking-wide text-muted-foreground">{isSelectable && !isSelectable(motion) ? "Renderer-Unterstützung ausstehend" : motion.safety === "SAFE" ? "Original bleibt fest" : motion.safety === "DEPTH_APPROXIMATION" ? "Tiefenmodell erforderlich" : "KI-Entwurf erforderlich"}</span>
                  </button>
                ))}
              </div>
            </details>
          ...
          <>
            ...
            >
              ...
              {elements.map((element) => (
                ...
                >
                  <TypographyPreviewContent element={element} durationSeconds={durationSeconds} />
                  {element.id === activeId && <>
                    <button
                      type="button"
...
export function AiStudioPanel({
  ...
  return (
    <section data-workflow-stage="preview_ai" className="space-y-6">
      <div className="flex flex-col gap-4 border-b border-border/70 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">5 · Vorschau & KI-Studio</p>
          <h2 className="mt-2 text-2xl font-semibold">Demo-Bewegung einem echten Motiv zuordnen</h2>
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Beispiel: Bild 3 + Demo 16. Der Entwurf ist kostenlos speicherbar; die eigentliche Generierung bleibt bis zur Providerfreigabe gesperrt.</p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(300px,.75fr)_minmax(0,1.25fr)]">
        <section className="space-y-4 border border-border/70 bg-card p-5">
          ...
          <p className="text-sm font-semibold">{selectedProposal ? `+ Idee ${selectedProposal.demoNumber} · ${selectedProposal.name}` : "+ Noch keine freigegebene Idee gewählt"}</p>
          <Control label="Timeline-Verhalten"><select value={activationMode} onChange={(event) => setActivationMode(event.target.value as typeof activationMode)}><option value="replace_take">Diesen Standbild-Slot ersetzen</option><option value="insert_after_take">Als neuen Clip dahinter einfügen</option></select></Control>
          <p className="border border-blue-300 bg-blue-50 p-3 text-xs text-blue-950">Ein fertiger Clip erhält ausschließlich serverseitig seine echte Dauer. Er ersetzt genau einen Slot oder wird ausdrücklich als eigener Slot eingefügt; die Dauer wird nie doppelt gezählt.</p>
        </section>
        <section className="space-y-4 border border-border/70 bg-card p-5">
          <h3 className="font-semibold">Geprüfte Ideenvorlagen</h3>
          <p className="border border-amber-300 bg-amber-50 p-3 text-xs text-amber-950">Eine Idee ist erst auswählbar, wenn sowohl ein freigegebenes Quellbeispiel als auch ein echtes Ergebnisvideo oder eine Ergebnisloop mit Dauer und Voraussetzungen vorliegt. Auftragsfotos werden nicht als KI-Ergebnisse ausgegeben.</p>
          <div className="grid max-h-[620px] gap-3 overflow-y-auto pr-1 md:grid-cols-2">
            {AI_STUDIO_PROPOSALS.map((proposal) => {
              ...
              return <button key={proposal.id} type="button" disabled={!selectable} onClick={() => setProposalId(proposal.id)} className={`border p-4 text-left disabled:cursor-not-allowed disabled:opacity-65 ${proposalId === proposal.id ? "border-primary ring-2 ring-primary/20" : "border-border"}`}>
                ...
                <span className="mt-2 block text-xs text-muted-foreground">Quellbeispiel: {proposal.sourceExampleUrl ? "vorhanden" : "noch nicht freigegeben"} · Ergebnis-{proposal.resultExampleKind ?? "clip/loop"}: {proposal.resultExampleUrl ? `${proposal.durationSeconds?.toFixed(1)} s` : "fehlt"} · Herkunft: {proposal.provenance ?? "nicht belegt"}</span>
      ...
      <section className="space-y-4 border border-border/70 bg-card p-5">
        ...
        <div className="flex flex-wrap gap-3"><Button disabled={busy || !sourceAssetId || !selectedProposal || !isSelectableAiStudioProposal(selectedProposal)} onClick={saveDraft}>Storyboard speichern</Button><Button disabled variant="outline">Generierung starten · noch gesperrt</Button></div>
        <p className="text-xs text-muted-foreground">Die endgültige Werkstatt- und Renderausgabe bleibt auf 60 fps festgelegt. Hier wird kein KI-Providerjob gestartet.</p>
      </section>
    </section>
  );
}
...
export function TypographyPreviewContent({ element, durationSeconds }: { element: StudioTypographyElement; durationSeconds: number }) {
